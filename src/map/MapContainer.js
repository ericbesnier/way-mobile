...
import * as  RegisterLocationUpdatesTask from './RegisterLocationUpdatesTask';
import {
...
class MapContainer extends Component {
  ...
  subscribeAndPostLocationUpdates = async () => {
    ...
    console.log('MapContainer/subscribeAndPostLocationUpdates: ', currentDate(), ' user.isTracking =', user.isTracking);
  ...
  render = () => {
    ...
    // console.log('MapContainer/render: user.isTracking=', user.isTracking);
    if (!appState.isLocationPermissionsFulfilled) {
    ...
    switch (locationPermissionsStatus) {
      case 'granted':
        ...
        return (
          <ErrorBoundary>
            <MapScreen style={{ flex: 2 }} navigation={this.props.navigation} />
          </ErrorBoundary>
        );
      case 'denied':
...
const mapDispatchToProps = (dispatch) => ({
  ...
  setTracking: (isTracking) => dispatch(_setTracking(isTracking)),
});

export default connect(mapStateToProps, mapDispatchToProps)(MapContainer);

// RegisterLocationUpdatesTask.registerLocationUpdatesTask(BACKGROUND_LOCATION_UPDATES_TASK);
